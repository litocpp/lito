export module tenon.preprocessor:expression;
...
private:
  auto failure(ref<str> message) -> Result<i64> {
    ...
    if (!tokens_.is_empty()) {
      return Err(Error::at(String::make(message),
                           tokens_[tokens_.len() - usize(1)].expansion));
    }
    return Err(Error::make(message));
  }

  auto match(ref<str> value) -> bool {
    if (index_ >= tokens_.len() || tokens_[index_].text.as_str() != value)
      return false;
    ++index_;
    return true;
  }
  ...
  auto conditional() -> Result<i64> {
    ...
    if (condition.is_err() || !match("?"_str))
      return condition;
    ...
    if (when_true.is_err())
      return when_true;
    if (!match(":"_str))
      return failure("expected ':' in conditional expression"_str);
    evaluating_ = outer && *condition == i64{};
    auto when_false = conditional();
    evaluating_ = outer;
    if (when_false.is_err())
      return when_false;
    if (!outer)
      return Ok(i64{});
  ...
  auto multiplicative() -> Result<i64> {
    ...
    while (left.is_ok()) {
      if (match("*"_str)) {
        ...
        left = evaluating_ ? Ok(remainder ? left->wrapping_rem(*right)
  ...
  auto primary() -> Result<i64> {
    if (match("("_str)) {
      ...
      if (value.is_err())
        return value;
      if (!match(")"_str))
        return failure("expected ')' in preprocessor expression"_str);
      return value;
    }
    if (index_ >= tokens_.len())
      return failure("expected preprocessor expression"_str);
    ...
    if (token.kind != TokenKind::PpNumber)
      return failure("expected integer constant"_str);
    auto bytes = token.text.as_str().as_bytes();
    ...
    if (bytes.len() > usize(1) && bytes[usize{}] == u8('0')) {
      ...
      cursor = usize(1);
      if (cursor < bytes.len() &&
          (bytes[cursor] == u8('x') || bytes[cursor] == u8('X'))) {
        base = 16;
        ++cursor;

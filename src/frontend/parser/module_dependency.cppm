...
auto parse_name(const Vec<lexical::Token> &tokens, usize start,
                ref<str> context) -> lexical::Result<Option<ParsedName>> {
  if (start >= tokens.len())
    return Ok(None());
  if (tokens[start].text.as_str() == "<"_str ||
    ...
    return frontend_failure<Option<ParsedName>>(rstd::format(
        "{} uses an unsupported header unit at line {}", context,
  ...
  if (index >= tokens.len() ||
      tokens[index].kind != lexical::TokenKind::Identifier) {
    return Ok(None());
  }
...
auto copied_path(const Option<rstd::path::PathBuf> &path)
    -> Option<rstd::path::PathBuf> {
  return path.is_some()
             ? Some(rstd::path::PathBuf::from(path->as_path()))
             : Option<rstd::path::PathBuf>{};
}
...
public:
  static auto make() -> ModuleDependencyConsumer {
    return ModuleDependencyConsumer{};
  }
  ...
  auto consume(Vec<lexical::Token> tokens) -> lexical::Result<empty> {
    for (auto &token : tokens) {
      ...
      if (candidate_.len() == usize(1) &&
          candidate_[usize{}].text.as_str() == "export"_str &&
          text != "module"_str && text != "import"_str) {
        candidate_ = Vec<lexical::Token>::make();
  ...
  auto finish(const preprocessor::PreprocessedTranslationUnit &translation)
    ...
    auto facts = FrontendResult{
        .source = rstd::path::PathBuf::from(
            translation.sources.path(translation.main_source)),
        .provided = rstd::move(provided_),
        .implementation_module = rstd::move(implementation_module_),
        .header_inputs = [&]() {
          auto paths = Vec<rstd::path::PathBuf>::with_capacity(
              translation.header_inputs.len());
...
private:
  auto parse_candidate() -> lexical::Result<empty> {
    ...
    if (exported)
      declaration = usize(1);
    if (declaration >= candidate_.len())
      return Ok(empty{});
    auto keyword = candidate_[declaration].text.as_str();
    if (keyword == "module"_str &&
        declaration + usize(1) < candidate_.len() &&
        candidate_[declaration + usize(1)].text.as_str() == ";"_str) {
      return Ok(empty{});
    }
    auto parsed = parse_name(candidate_, declaration + usize(1),
                             keyword == "module"_str
    ...
    if (keyword == "module"_str) {
      ...
      if (exported) {
        provided_ = Some(ProvidedModule{
            .logical_name = rstd::move(name), .is_interface = true});
      } else if (name.as_str().contains(":"_str)) {
        provided_ = Some(ProvidedModule{
            .logical_name = rstd::move(name), .is_interface = false});
      } else {
    ...
    if (import_names_.contains_key(logical_name.as_str()))
      return Ok(empty{});
...
auto parse_module_dependencies(
  ...
  if (consumed.is_err())
    return Err(rstd::move(consumed).unwrap_err());

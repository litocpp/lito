...
auto literal_prefix_length(slice<u8> bytes, usize index) -> usize {
  ...
  if (index + usize(1) < bytes.len() &&
      (bytes[index] == u8('u') || bytes[index] == u8('U') ||
       bytes[index] == u8('L')) &&
...
auto lex(const SourceFile &source, bool borrow_spelling = false)
  ...
  while (index < bytes.len()) {
    ...
    if (value == u8(' ') || value == u8('\t') || value == u8('\f') ||
        value == u8('\v')) {
      ++index;
      ++column;
      pending_space = true;
    ...
    if (raw_prefix != usize{}) {
      ...
      if (!closed)
        return lex_failure("unterminated raw string literal"_str, location);
    } else if (value == u8('"') || value == u8('\'') || prefix != usize{}) {
      index += prefix;
      column += prefix;
      auto quote = bytes[index];
      kind = quote == u8('"') ? TokenKind::StringLiteral
      ...
      if (!closed)
        return lex_failure("unterminated literal"_str, location);
    } else if (is_identifier_start(value) ||
               ucn_length(bytes, index) != usize{}) {
      ...
      while (index < bytes.len()) {
        ...
        ++column;
               ...
               (value == u8('.') && index + usize(1) < bytes.len() &&
                bytes[index + usize(1)] >= u8('0') &&
                bytes[index + usize(1)] <= u8('9'))) {
      kind = TokenKind::PpNumber;
      while (index < bytes.len()) {
        auto current = bytes[index];
        if (is_identifier_continue(current) || current == u8('.') ||
            current == u8('\'')) {
          ++index;
          ++column;
          continue;
        ...
        if ((current == u8('+') || current == u8('-')) &&
            index != token_start &&
            (bytes[index - usize(1)] == u8('e') ||
             bytes[index - usize(1)] == u8('E') ||
             bytes[index - usize(1)] == u8('p') ||
             bytes[index - usize(1)] == u8('P'))) {
          ++index;
          ++column;
          continue;

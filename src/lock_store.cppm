...
auto required_member(const Json& value,
                     ref<str> key,
                     ref<str> context) -> Result<ref<Json>> {
...
auto required_string(const Json& value,
                     ref<str> key,
                     ref<str> context) -> Result<ref<str>> {
...
auto load_existing(ref<rstd::path::Path> path) -> Result<Option<Json>> {
    ...
    if (exists.is_err()) {
        return failure<Option<Json>>(rstd::format(
    ...
    auto valid = number.is_some() && *number == u64(1)
...
auto sync_lock(const ResolvedPackageGraph& graph, bool locked) -> Result<LockStatus> {
    ...
    if (locked) {
        ...
        if (existing_result.is_err()) {
            return Err(rstd::move(existing_result).unwrap_err());
        }
    ...
    auto existing_result = load_existing(destination.as_path());
    if (existing_result.is_err()) return Err(rstd::move(existing_result).unwrap_err());
    auto existing = rstd::move(existing_result).unwrap();
    if (existing.is_some() && *existing == desired) {
        return Ok(LockStatus::Unchanged);
    }

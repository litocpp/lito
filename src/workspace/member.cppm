...
template <typename T> auto workspace_failure(String message) -> Result<T> {
  return Err(Error::make(ErrorKind::Manifest, rstd::move(message)));
}

auto same_path(ref<rstd::path::Path> left, ref<rstd::path::Path> right) noexcept
    -> bool {
...
auto resolve_containing_workspace_version(PackageManifest &manifest)
    -> Result<empty> {
  if (manifest.version.value.is_some()) {
    return Ok(empty{});
  }
  ...
  while (directory.pop()) {
    ...
    if (located.is_err())
      return Err(rstd::move(located).unwrap_err());
    if (located->is_none())
      continue;

    auto document = load_manifest_document(directory.as_path());
    if (document.is_err())
      return Err(rstd::move(document).unwrap_err());
    auto loaded = rstd::move(document).unwrap();
    if (loaded.kind != ManifestKind::Workspace || loaded.workspace.is_none())
      continue;
    auto workspace = rstd::move(loaded.workspace).unwrap();
    for (const auto &declared : workspace.members) {
      auto member = workspace_member_directory(workspace, declared.as_path(),
                                               "workspace member"_str);
      if (member.is_err())
        return Err(rstd::move(member).unwrap_err());
      if (!same_path(member->as_path(), manifest.root.as_path()))
        continue;
  ...
  return workspace_failure<empty>(
      rstd::format("package '{}' inherits package.version but no containing "
                   ...
                   manifest.name.as_str(), manifest.root.as_path()));
}

...
auto same_path(rstd::ref<rstd::path::Path> left,
               rstd::ref<rstd::path::Path> right) noexcept -> bool {
    return left.starts_with(right) && right.starts_with(left);
}

auto extension_text(rstd::ref<rstd::path::Path> path) -> rstd::Option<rstd::ref<rstd::str>> {
    auto extension = path.extension();
    if (extension.is_none()) return rstd::None();
    return (*extension).to_str();
}

auto supported_source(rstd::ref<rstd::path::Path> path) -> bool {
...
auto module_interface(rstd::ref<rstd::path::Path> path) -> bool {
    auto extension = extension_text(path);
    return extension.is_some() && *extension == "cppm"_str;
}

auto valid_segment(rstd::ref<rstd::str> value) -> bool {
    ...
    for (rstd::usize index {}; index < value.size(); ++index) {
        ...
        if ((! alpha && ! digit) || (index == rstd::usize {} && digit)) return false;
    }
    return true;
}

auto path_segments(rstd::ref<rstd::path::Path> path) -> Result<Vec<String>> {
...
auto expected_module(const PackageManifest& manifest,
    ...
    const auto directory_count = segments.len() - rstd::usize(1);
    for (rstd::usize index {}; index < directory_count; ++index) {
    ...
    if (! directory_marker) {
        auto stem = file.as_str().split_at(file.len() - rstd::usize(5)).template get<0>();
    ...
    return rstd::Ok(rstd::move(logical_name));
}
...
auto append_source(const PackageManifest& manifest,
                   rstd::ref<rstd::path::Path> source_root,
                   rstd::ref<rstd::path::Path> requested,
                   StringMap& expected_paths,
                   Vec<SourceEntry>& entries) -> Result<rstd::empty> {
    ...
    if (module_interface(resolved.as_path())) {
        ...
        expected = rstd::Some(rstd::move(logical));
    }
...
auto walk_sources(const PackageManifest& manifest,
                  rstd::ref<rstd::path::Path> source_root,
                  rstd::ref<rstd::path::Path> directory,
                  StringMap& expected_paths,
                  Vec<SourceEntry>& entries) -> Result<rstd::empty> {
...
auto validate_module_conventions(const PackageSpec& package,
                                 ...
                                 const Vec<ScanResult>& scans) -> Result<rstd::empty> {
    using namespace module_convention_detail;

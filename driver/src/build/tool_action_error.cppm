...
class BuildToolActionError {
    RSTD_ENUM(BuildToolActionError,
              ...
              (Publication, (String operation; PathBuf path; rstd::io::error::Error source;)),
              (Receipt, (String operation; PathBuf path; rstd::io::error::Error source;)))
};

template<typename T>
using BuildToolActionResult = Result<T, BuildToolActionError>;
...
template<>
struct Impl<fmt::Display, lito::BuildToolActionError> : ImplBase<lito::BuildToolActionError> {
    auto fmt(fmt::Formatter& formatter) const -> bool {
        ...
        if (error.is_InvalidInput()) {
            ...
            return formatter.write_str(rstd::format("build-tool action input '{}': {}",
                                                    value.path.as_path(),
                                                    value.reason.as_str())
                                           .as_str());
        }
        if (error.is_InvalidOutput()) {
            ...
            return formatter.write_str(rstd::format("build-tool action output '{}': {}",
                                                    value.path.as_path(),
                                                    value.reason.as_str())
                                           .as_str());
        }
        if (error.is_Execution()) {
            ...
            return formatter.write_str(rstd::format("build-tool '{}' exited with {}\n{}",
                                                    value.alias.as_str(),
                                                    value.exit_code,
        ...
        if (error.is_Process()) {
            return formatter.write_str(
                rstd::format("cannot execute build-tool '{}'", error.as_Process().alias.as_str())
                    .as_str());
        }
...
template<>
struct Impl<fmt::Debug, lito::BuildToolActionError> : ImplBase<lito::BuildToolActionError> {
    auto fmt(fmt::Formatter& formatter) const -> bool {
...
template<>
struct Impl<error::Error, lito::BuildToolActionError> : ImplBase<lito::BuildToolActionError> {

export module lito.driver:build.profiling;
...
public:
    ...
    auto finish() noexcept -> Result<empty, rstd::bench::probe::ProbeError> {
...
class ScanTaskProfiler {
    ...
    auto probe(ScanProbe value) const noexcept -> rstd::bench::probe::ProbeId {
        return probes_[usize(static_cast<size_t>(value))];
    }
...
public:
    ...
    auto complete(ScanSpanGuard& span) -> Result<empty, String> {
...
public:
    static auto create() -> Result<ScanProfiler, String> {
        auto registry = rstd::bench::probe::ProbeRegistry::new_();
        auto probes   = Vec<rstd::bench::probe::ProbeId>::with_capacity(
            usize(static_cast<size_t>(ScanProbe::Count)));
        for (auto index = size_t {}; index < static_cast<size_t>(ScanProbe::Count); ++index) {
            auto registered =
    ...
    auto ingest(ScanTaskProfile profile) -> Result<empty, String> {
    ...
    auto end_source_frame() -> Result<empty, String> {
    ...
    auto finish() -> Result<ScanProfileReport, String> {

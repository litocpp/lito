...
auto registry_install_source_key(ref<str> key) -> bool {
    return key == "checksum"_str || key == "identity"_str || key == "kind"_str ||
           key == "package"_str || key == "registry"_str || key == "version"_str;
}
...
auto parse_install_source_provenance(const Json& source)
    ...
    if (kind == "registry"_str) {
        ...
        auto checksum_text = rstd_try(required_source_string(source, "checksum"_str));

...
auto is_dynamic_builtin_name(ref<str> name) -> bool {
  return name == "__LINE__"_str || name == "__FILE__"_str ||
         ...
         name == "__has_cpp_attribute"_str ||
         name == "__has_attribute"_str ||
         name == "__has_declspec_attribute"_str ||
         name == "__has_embed"_str || name == "__has_warning"_str ||
         name == "__is_identifier"_str ||
         name == "__is_target_arch"_str ||
...
struct MacroDefinition {
  ...
  auto is_dynamic_builtin() const noexcept -> bool {
    return dynamic_builtin_name_;
  }

  auto parameter_index(ref<str> value) const -> Option<usize> {
    if (parameters.is_some()) {
      for (auto index = usize{}; index < parameters->len(); ++index) {
        if ((*parameters)[index].as_str() == value)
          return Some(index);
      }
    }
    if (variadic &&
        (value == "__VA_ARGS__"_str || value == variadic_name.as_str())) {
  ...
  auto set_replacement(Vec<Token> value) -> void {
    ...
    if (variadic)
      ++count;
    expanded_parameter_last_uses_ =
        Vec<Option<usize>>::with_capacity(count);
    ...
    for (auto index = usize{}; index < count; ++index)
      unexpanded_parameter_uses_.push(false);
    ...
    for (auto index = usize{}; index < replacement.len(); ++index) {
      if (variadic && replacement[index].text.as_str() == "__VA_OPT__"_str &&
        ...
        auto depth = usize{};
        for (auto cursor = index + usize(2); cursor < replacement.len();
             ++cursor) {
      ...
      if (parameter.is_none())
        continue;
      auto paste_left = index > usize{} &&
                        replacement[index - usize(1)].text.as_str() == "##"_str;
      auto paste_right = index + usize(1) < replacement.len() &&
                         replacement[index + usize(1)].text.as_str() == "##"_str;
      auto stringified = index > usize{} &&
                         replacement[index - usize(1)].text.as_str() == "#"_str;
  ...
  auto is_last_expanded_use(usize parameter, usize replacement_index) const
      -> bool {
  ...
  auto va_opt_end(usize replacement_index) const -> Option<usize> {
    if (replacement_index >= va_opt_ends_.len())
      return None();

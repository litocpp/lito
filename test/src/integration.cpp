...
TEST(Integration, ScanUsesNativePreprocessorAndDefinitions) {
  ...
  EXPECT_EQ(native->result.provided->logical_name.as_str(),
            "fixture.preprocessor.native"_str);
  EXPECT_TRUE(
      has_import(*native, "fixture.preprocessor.native:dependency"_str));
  EXPECT_FALSE(has_import(
      *native, "fixture.preprocessor.native:native_builtin_failure"_str));
  ...
  EXPECT_FALSE(
      has_import(*definitions, "fixture.scan.definitions:missing"_str));
  EXPECT_FALSE(has_import(
      *definitions, "fixture.scan.definitions:command_line_undef_failure"_str));
}
...
TEST(Integration, BuildSelectsProductionArtifacts) {
  ...
  auto request =
      build_request(root.as_path(), output.as_path(),
                    strings("fixture-test-lib"_str, "fixture-test-app"_str));
  auto summary = tenon::build(request);
  ASSERT_TRUE(summary.is_ok());
  EXPECT_EQ(artifact_count(*summary, tenon::ArtifactKind::StaticLibrary),
            usize(1));
  EXPECT_EQ(artifact_count(*summary, tenon::ArtifactKind::Executable),
            usize(1));
  EXPECT_EQ(artifact_count(*summary, tenon::ArtifactKind::TestExecutable),
            usize{});
  clear_output(output.as_path());
}
...
TEST(Integration, TestRunsPassFailureSignalAndNoRun) {
  ...
  auto pass_request = tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-test-pass"_str)),
  ...
  auto no_run_request = tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-test-pass"_str,
                                     "fixture-test-fail"_str,
                                     "fixture-test-signal"_str),
  ...
  EXPECT_EQ(artifact_count(no_run->build, tenon::ArtifactKind::TestExecutable),
            usize(3));

  auto failure = tenon::test(tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-test-fail"_str)),
  });
  ...
  auto signal = tenon::test(tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-test-signal"_str)),
  });
...
TEST(Integration, CompileTestsReportOutcomesAndReuse) {
  ...
  auto passed = tenon::test(tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-compile-pass"_str)),
  });
  ASSERT_TRUE(passed.is_ok());
  EXPECT_TRUE(passed->success());
  ...
  auto mismatch = tenon::test(tenon::TestRequest{
      .build = build_request(root.as_path(), output.as_path(),
                             strings("fixture-compile-mismatch"_str)),
  });
  ...
  auto unsupported = tenon::build(build_request(
      root.as_path(), output.as_path(), strings("fixture-windows-only"_str)));
...
TEST(Integration, EnvironmentIsSharedWithinBuild) {
  ...
  clear_output(output.as_path());
  auto summary =
      tenon::build(build_request(root.as_path(), output.as_path(),
                                 strings("fixture-environment-cache"_str)));
  ASSERT_TRUE(summary.is_ok());
  ...
  auto emitted =
      tenon::timing_output::emit(*summary, tenon::timing_output::OutputOptions{
...
TEST(Integration, ScanCacheReusesAndInvalidatesOwnedInputs) {
  ...
  auto cold = tenon::build(
      build_request(fixture.as_path(), output.as_path(), Vec<String>::make()));
  ...
  auto staged_optional =
      fixture.join(PathBuf::from("staged/optional.hpp"_str).as_path());
  auto high_optional =
      fixture.join(PathBuf::from("high/optional.hpp"_str).as_path());
  ASSERT_TRUE(rstd::fs::copy(staged_optional.as_path(), high_optional.as_path())
                  .is_ok());
  auto optional = tenon::build(
      build_request(fixture.as_path(), output.as_path(), Vec<String>::make()));
  ...
  auto staged_priority =
      fixture.join(PathBuf::from("staged/choice.hpp"_str).as_path());
  auto high_priority =
      fixture.join(PathBuf::from("high/choice.hpp"_str).as_path());
  ASSERT_TRUE(rstd::fs::copy(staged_priority.as_path(), high_priority.as_path())
                  .is_ok());
  auto priority = tenon::build(
      build_request(fixture.as_path(), output.as_path(), Vec<String>::make()));
  ASSERT_TRUE(priority.is_ok());
  EXPECT_EQ(priority->frontend.persistent_scan_include_lookup, usize(1));

  auto staged_header =
      fixture.join(PathBuf::from("staged/choice-low.hpp"_str).as_path());
  auto low_header = fixture.join(PathBuf::from("low/choice.hpp"_str).as_path());
  ASSERT_TRUE(
      rstd::fs::copy(staged_header.as_path(), low_header.as_path()).is_ok());
  auto header = tenon::build(
      build_request(fixture.as_path(), output.as_path(), Vec<String>::make()));
  ASSERT_TRUE(header.is_ok());
  EXPECT_EQ(header->frontend.persistent_scan_file_dependency, usize(1));

  auto staged_source =
      fixture.join(PathBuf::from("staged/lib.cppm"_str).as_path());
  auto primary_source =
      fixture.join(PathBuf::from("src/lib.cppm"_str).as_path());
  ASSERT_TRUE(rstd::fs::copy(staged_source.as_path(), primary_source.as_path())
                  .is_ok());
  auto changed_source = tenon::build(
      build_request(fixture.as_path(), output.as_path(), Vec<String>::make()));
  ...
  auto dynamic_fixture =
      dynamic_base.join(PathBuf::from("fixture"_str).as_path());
  auto dynamic_output =
      dynamic_base.join(PathBuf::from("output"_str).as_path());
  auto dynamic_source = root("cache/dynamic"_str);
  ASSERT_TRUE(
      copy_directory(dynamic_source.as_path(), dynamic_fixture.as_path()));
  auto dynamic_cold = tenon::build(build_request(dynamic_fixture.as_path(),
                                                 dynamic_output.as_path(),
                                                 Vec<String>::make()));
  ASSERT_TRUE(dynamic_cold.is_ok());
  auto dynamic_warm = tenon::build(build_request(dynamic_fixture.as_path(),
                                                 dynamic_output.as_path(),
                                                 Vec<String>::make()));
...
TEST(Integration, InvalidArtifactsAndDependenciesAreRejected) {
  auto multiple = tenon::load_manifest_document(
      root("test-command/invalid-artifact"_str).as_path());
  ...
  clear_output(output.as_path());
  auto request =
      build_request(invalid.as_path(), output.as_path(), Vec<String>::make());

...
TEST(Integration, DocumentationUsesFrontendFactsAndPublishesVersionedOutput) {
    ...
    auto generated = lito::generate_documentation(lito::DocRequest {
        .selection =
            lito::PackageSelection {
                .root     = project.clone(),
    ...
    EXPECT_TRUE(data_manifest->as_str().contains("\"title\": \"lito-test-project\""_str));
    EXPECT_TRUE(lito::doc::validate_data(data.as_path()).is_ok());
    ...
    auto corrupt_validation = lito::doc::validate_data(corrupt_data.as_path());
    ...
    EXPECT_TRUE(lito::doc::validate_json(json->as_str()).is_ok());
    EXPECT_TRUE(lito::doc::validate_json("{\"format\":\"lito-doc\",\"version\":2}"_str).is_err());
    ...
    auto restored = lito::render_documentation(lito::DocRenderRequest {
    ...
    EXPECT_TRUE(
        original_index->as_str().contains("<title>Overview · lito-test-project</title>"_str));
    EXPECT_TRUE(original_index->as_str().contains("<h1>lito-test-project</h1>"_str));
    EXPECT_FALSE(original_index->as_str().contains("Workspace documentation"_str));
    EXPECT_TRUE(original_index->as_str().contains("<lito-doc-shell>"_str));
    EXPECT_TRUE(original_index->as_str().contains("<lito-doc-search root-prefix="_str));
    ...
    EXPECT_TRUE(original_index->as_str().contains("<lito-doc-theme-picker>"_str));
    ...
    EXPECT_TRUE(symbol_pages.as_str().contains("<lito-doc-module-identity"_str));
    ...
    ASSERT_TRUE(rstd::fs::write_atomic(root_template.as_path(), marked_template.as_str().as_bytes())
                    .is_ok());
    auto custom = lito::render_documentation(lito::DocRenderRequest {
        .working_directory = project.clone(),
        .data              = data.clone(),
    ...
    ASSERT_TRUE(
        rstd::fs::write_atomic(root_template.as_path(), ("{{missing.value}}"_str).as_bytes())
            .is_ok());
    auto missing_value = lito::render_documentation(lito::DocRenderRequest {
        .working_directory = project.clone(),
        .data              = data.clone(),
        .output            = custom_output.clone(),
        .frontend          = Some(custom_frontend.clone()),
    ...
    ASSERT_TRUE(rstd::fs::write_atomic(root_template.as_path(),
                    ...
                    .is_ok());
    auto partial_cycle = lito::render_documentation(lito::DocRenderRequest {
        .working_directory = project.clone(),
        .data              = data.clone(),
        .output            = custom_output.clone(),
        .frontend          = Some(custom_frontend.clone()),
    ...
    EXPECT_TRUE(lito::render_documentation(lito::DocRenderRequest {
                                                .working_directory = project.clone(),
                                                .data              = data.clone(),
                                                .output            = custom_output.clone(),
                                                .frontend          = Some(custom_frontend.clone()),
    ...
    auto data_only = lito::generate_documentation(lito::DocRequest {
        .selection =
            lito::PackageSelection {
                .root     = project.clone(),
                .packages = strings("fixture-doc-basic"_str),
    ...
    EXPECT_TRUE(lito::doc::validate_data(data_only_output.as_path()).is_ok());
    auto regenerated = lito::generate_documentation(lito::DocRequest {
        .selection =
            lito::PackageSelection {
                .root     = project.clone(),
                .packages = strings("fixture-doc-basic"_str),
    ...
    auto rejected = lito::generate_documentation(lito::DocRequest {
        .selection =
            lito::PackageSelection {
                .root     = project.clone(),

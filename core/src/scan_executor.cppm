export module lito.scan_executor;
...
struct FrontendScanTaskResult {
    usize                               node {};
    Result<FrontendAnalysisTaskOutcome> outcome;
};
...
public:
    ...
    static auto create(usize jobs, usize max_in_flight) -> Result<FrontendScanExecutor> {
        if (jobs == usize {} || max_in_flight == usize {}) {
            return Err(Error::make(ErrorKind::InvalidRequest,
                                   "scan execution requires non-zero jobs and capacity"_str));
        }
        ...
        if (pool.is_err()) {
            return Err(Error::make(ErrorKind::Artifact,
                                   rstd::format("cannot create scan worker pool: {}",
        ...
        if (tasks.is_err()) {
            return Err(Error::make(ErrorKind::Artifact,
                                   rstd::format("cannot create scan task set: {}",
                                                rstd::move(tasks).unwrap_err_unchecked())));
        }
        return Ok(FrontendScanExecutor(rstd::move(value),
                                       rstd::move(tasks).unwrap_unchecked(),
                                       SharedExecutionState::make(
                                           ExecutionFields { jobs, max_in_flight })));
    }

    auto submit(usize node, FrontendAnalysisTask task) -> Result<empty> {
        ...
        if (error == rstd::thread::BlockingTaskSetSubmitError::Full) {
            return Err(Error::make(ErrorKind::Artifact, "scan task set is full"_str));
        }
        if (error == rstd::thread::BlockingTaskSetSubmitError::Cancelled) {
            return Err(Error::make(ErrorKind::Artifact, "scan task set is cancelled"_str));
        }
        return Err(Error::make(ErrorKind::Artifact, "scan task set is closed"_str));
    }

    auto recv() -> Result<FrontendScanTaskResult> {
        auto started    = rstd::time::Instant::now();
        ...
        if (completion.is_none()) {
            return Err(Error::make(ErrorKind::Artifact,
                                   "scan task set closed before a completion arrived"_str));
        }
        auto value = rstd::move(completion).unwrap_unchecked();
        if (value.is_cancelled()) {
            return Err(Error::make(ErrorKind::Artifact, "scan task was cancelled"_str));
        }
        auto result = rstd::move(value).into_value();
        if (result.is_none()) {
            return Err(
                Error::make(ErrorKind::Artifact, "scan task completed without a result"_str));
        }
...
public:
    ...
    static auto create(usize jobs, usize max_in_flight) -> Result<FrontendScanExecution> {
        if (jobs == usize {} || max_in_flight == usize {}) {
            return Err(Error::make(ErrorKind::InvalidRequest,
                                   "scan execution requires non-zero jobs and capacity"_str));
        }
    ...
    auto prepare(usize ready) -> Result<empty> {
        if (ready == usize {}) {
            return Err(Error::make(ErrorKind::Artifact,
                                   "scan execution cannot prepare an empty frontier"_str));
        }
    ...
    auto submit(usize node, FrontendAnalysisTask task) -> Result<empty> {
        if (executor_.is_none()) {
            return Err(
                Error::make(ErrorKind::Artifact, "scan execution has no prepared frontier"_str));
        }

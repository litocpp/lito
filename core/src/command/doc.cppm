...
export module lito.command.doc;

import rstd;
import rstd.json;
import lito.error;
export import lito.command.doc_contract;
import lito.command.doc_tool;
import lito.build;
...
auto selected_package(const BuildSummary& summary, ref<str> name)
    -> Option<ref<SelectedPackageMetadata>> {
    for (const auto& package : summary.selected_packages) {
        if (package.name.as_str() == name) {
            return Some(ref<SelectedPackageMetadata>::from_raw_parts(rstd::addressof(package)));
        }
    }
    return None();
}
...
auto extraction_request_json(const BuildSummary&            summary,
                             ...
                             const SelectedPackageMetadata& package,
...
auto site_manifest_json(const BuildSummary&     summary,
    ...
    root.insert(String::make("frontend"_str),
                Json::String(rstd_try(
                    doc_path_text(frontend, "documentation frontend"_str))));
...
auto doc(DocRequest request) -> DocResult<DocSummary> {
    ...
    auto frontend = request.frontend.is_some() ? request.frontend->clone()
                                               : tool->default_frontend.clone();

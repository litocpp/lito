...
export module lito.command.doc_tool;
...
import lito.command.doc_contract;
...
inline constexpr auto LITODOC_COMMIT     = "2575f4ba5448054f9ba6ac45780f69f6c7efb750"_str;
...
auto emit_doc(const Option<DocObserver>& observer,
              DocEventKind               kind,
              ref<str>                   target,
...
auto acquire_doc_tool_source(const BuildRequest&               request,
    ...
    auto options = PackageResolutionOptions {
        .locked = false,
        .git    = GitResolutionMode::ReuseLocked,
        .sources =
            PackageSourceConfig {
                .fetch_seeds = as<rstd::clone::Clone>(request.sources.fetch_seeds).clone(),
    ...
    auto manager = SourceManager(request.selection.root.as_path(),
                                 ...
                                 resolver,
                                 environment,
                                 request.observer.is_some() ? *request.observer : BuildObserver {});
...
auto write_tool_receipt(ref<rstd::path::Path>      path,
    ...
    if (frontend_text.is_none()) {
        return doc_tool_failure<empty>(rstd::format(
            "litodoc default frontend '{}' is not valid UTF-8", default_frontend));
    }
    ...
    root.insert(String::make("default_frontend"_str),
                Json::String(String::make(*frontend_text)));
...
auto copy_doc_tool_file(ref<rstd::path::Path> source,
                        ref<rstd::path::Path> destination) -> DocResult<empty> {
...
auto published_doc_tool(ref<rstd::path::Path>      tool_root,
    ...
    auto identity = rstd::crypto::sha256_hex(
        rstd::format("litodoc-artifact-v1\n{}\n{}",
                     executable_digest,
                     frontend.identity.as_str()).as_str());
    ...
    if (*exists) {
        ...
        if (removed.is_err()) {
            return Err(doc_io_failure(
                "replace invalid litodoc artifact"_str,
                final.as_path(),
    ...
    if (created.is_err()) {
        return Err(doc_io_failure(
            "create litodoc artifact store"_str,
            artifacts.as_path(),
    ...
    if (staging_exists.is_err()) {
        return Err(doc_io_failure(
            "inspect litodoc artifact staging"_str,
            staging.as_path(),
    ...
    if (*staging_exists) {
        ...
        if (removed.is_err()) {
            return Err(doc_io_failure(
                "clear litodoc artifact staging"_str,
                staging.as_path(),
                rstd::move(removed).unwrap_err()));
        }
    }
    ...
    if (staged_frontend_identity.is_err()) {
        return doc_tool_failure<PublishedDocTool>(
            rstd::format("cannot validate staged litodoc frontend: {}",
                         staged_frontend_identity.unwrap_err()));
    }
...
auto resolve_doc_tool(const BuildRequest&               request,
                      ...
                      const ResolvedProcessEnvironment& environment,
                      const Option<DocObserver>&        observer) -> DocResult<ResolvedDocTool> {
    ...
    tool_request.sources.fetch_seeds = as<rstd::clone::Clone>(request.sources.fetch_seeds).clone();
    ...
    for (const auto& resource : built->runtime_resources) {
        if (resource.target.kind == PackageTargetKind::Binary &&
            ...
            default_frontend = Some(
                ref<BuiltRuntimeResource>::from_raw_parts(rstd::addressof(resource)));
        }
    }

...
public:
    ...
    auto preprocess_documentation(ref<rstd::path::Path>      source,
                                  const CompileContext&      compile_context,
                                  ref<rstd::path::Path>      working_directory,
                                  frontend::FrontendService& frontend_service,
                                  ScanProfiler&              profiler) const
        -> Result<frontend::UncachedDocumentationAnalysis> {
        auto frontend_span = profiler.span(ScanProbe::Frontend);
        frontend_service.record_analysis_build();
        auto environment_span = profiler.span(ScanProbe::Environment);
        for (const auto& option : compile_context.options) {
        ...
        auto pragmas           = toolchain::ClangPragmaHandler {};
        auto events            = toolchain::DependencyEvents {};
        auto consumer          = frontend::preprocessor::CollectPreprocessedTokens {};
        auto observer          = PreprocessorProfileObserver(profiler);
        auto translation       = profiler.measure(ScanProbe::Preprocessor, [&] {
        ...
        if (translation.is_err()) {
            ...
            return failure<frontend::UncachedDocumentationAnalysis>(rstd::format(
                "native preprocessing failed for '{}': {}", source, error.message.as_str()));
        }
        translation->tokens        = consumer.take();
        translation->header_inputs = events.take_headers();
        auto dependencies          = frontend::parser::parse_module_dependencies(*translation);
        if (dependencies.is_err()) {
            return failure<frontend::UncachedDocumentationAnalysis>(
                rstd::move(dependencies).unwrap_err().message.clone());
        }
        auto documentation = frontend::parser::parse_documentation(*translation, *dependencies);
        if (documentation.is_err()) {
            return failure<frontend::UncachedDocumentationAnalysis>(
                rstd::move(documentation).unwrap_err().message.clone());
        }
        return Ok(frontend::UncachedDocumentationAnalysis {
            .result          = rstd::move(dependencies).unwrap(),
            .include_lookups = includes.take_dependencies(),

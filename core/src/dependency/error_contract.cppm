...
export module lito.dependency.error_contract;
...
class DependencyError {
    RSTD_ENUM(DependencyError,
              ...
              (Cpp, (String dependency; CppOptionError source;)),
              (CMakeOperation,
               (String dependency; String operation; PathBuf work_area;
                rstd::boxed::Box<DependencyError> source;)),
              (Message, (String message;)))
};

template<typename T>
using DependencyResult = rstd::Result<T, DependencyError>;
...
template<>
struct Impl<convert::From<lito::SystemError>, lito::DependencyError> {
    static auto from(lito::SystemError error) -> lito::DependencyError {
...
struct Impl<fmt::Display, lito::DependencyError> : ImplBase<lito::DependencyError> {
    auto fmt(fmt::Formatter& formatter) const -> bool {
        ...
        if (error.is_Json()) {
            ...
            return formatter.write_fmt(fmt::Arguments::make(
                "cannot parse {} '{}'", value.context, value.path.as_path()));
        }
        if (error.is_Cpp()) {
            return formatter.write_fmt(fmt::Arguments::make(
                "dependency '{}' C++ options are invalid", error.as_Cpp().dependency));
        }
        if (error.is_CMakeOperation()) {
            const auto& value = error.as_CMakeOperation();
            return formatter.write_fmt(fmt::Arguments::make("CMake dependency '{}' {} failed in '{}'",
                                                              value.dependency,
                                                              value.operation,

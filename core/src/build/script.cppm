...
public:
    ...
    auto configure(ref<str>               package_name,
        ...
        auto rendered = render_configure_template(template_text->as_str(), values, input->as_path());
        if (rendered.is_err()) return Err(rstd::into<BuildScriptError>(rstd::move(rendered).unwrap_err()));
...
auto action_receipt_matches(ref<rstd::path::Path>      receipt,
                            ...
                            ref<rstd::path::Path>      generated_root)
...
auto action_receipt_text(ref<str>              identity,
                         const Vec<PathBuf>&   outputs,
                         const Vec<String>&    digests) -> String {
...
auto collect_action_outputs(ref<rstd::path::Path> root,
    ...
    if (opened.is_err()) {
        return script_io_failure<empty>(
            "enumerate staged build-tool outputs"_str,
            directory,
    ...
    for (auto next = entries.next(); next.is_some(); next = entries.next()) {
        ...
        if (type.is_err()) {
            return script_io_failure<empty>(
                "inspect staged build-tool output"_str,
                path.as_path(),
...
public:
    ToolActionSession(const PackageMetadata&             metadata,
                      const BuildLayout&                 layout,
                      ref<str>                           profile,
    ...
    auto run(const luato::Table& request) -> BuildScriptResult<bool> {
        ...
        if (handle.is_err()) return action_request_failure<bool>(rstd::format("{}", handle.unwrap_err()));
        if (package.is_err()) return action_request_failure<bool>(rstd::format("{}", package.unwrap_err()));
        if (cwd.is_err()) return action_request_failure<bool>(rstd::format("{}", cwd.unwrap_err()));
        if (args.is_err()) return action_request_failure<bool>(rstd::format("{}", args.unwrap_err()));
        if (inputs.is_err()) return action_request_failure<bool>(rstd::format("{}", inputs.unwrap_err()));
        if (outputs.is_err()) return action_request_failure<bool>(rstd::format("{}", outputs.unwrap_err()));
        ...
        if ((**resolved_tool).package != package->as_str()) {
            return action_request_failure<bool>(rstd::format(
                "build-tool '{}' belongs to package '{}', not '{}'",
                (**resolved_tool).alias.as_str(), (**resolved_tool).package.as_str(),
                package->as_str()));
        }
        ...
        auto cwd_relative = rstd_try(normal_relative_path(rstd::move(cwd).unwrap(),
                                                          "build-tool action cwd"_str));
        ...
        for (auto& input : declared_inputs) {
            auto relative = rstd_try(normal_relative_path(rstd::move(input),
                                                          "build-tool action input"_str));
        ...
        for (auto& output : declared_outputs) {
            auto relative = rstd_try(normal_relative_path(rstd::move(output),
                                                          "build-tool action output"_str));
        ...
        auto action_root = layout_->build_tool_action_root().join(PathBuf::from(identity.clone()).as_path());
        ...
        auto opened = rstd::fs::OpenOptions::make()
                          .read(true)
                          .write(true)
                          .create(true)
        ...
        if (locked.is_err()) {
            return script_io_failure<bool>("lock build-tool action"_str,
                                           lock_path.as_path(),
                                           rstd::move(locked).unwrap_err());
        }
        auto reusable = rstd_try(action_receipt_matches(receipt.as_path(),
                                                        identity.as_str(),
                                                        output_paths,
        ...
        if (staged_text.is_none()) {
            return action_request_failure<bool>(
                "build-tool staging path is not valid UTF-8"_str);
        }
        ...
        auto executed = run_command(invocation,
                                    *environment_,
        ...
        if (executed->exit_code != i32 {}) {
            return action_failure<bool>(BuildToolActionError::Execution(
                (**resolved_tool).alias.clone(),
                executed->exit_code,
...
auto execute_build_script(PackageMetadata&             metadata,
    ...
    auto resolved_tools = resolve_host_build_tools(metadata,
                                                   selected_packages,
                                                   host,
                                                   layout,
                                                   cmake,
                                                   ...
                                                   sources,
                                                   jobs,
                                                   observer.is_some() ? *observer : BuildObserver {});

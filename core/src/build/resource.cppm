...
export module lito.build.resource;
...
auto resource_identity(ref<rstd::path::Path> root, const Vec<PathBuf>& files)
    ...
    for (const auto& relative : files) {
        ...
        if (text.is_none()) {
            return Err(BuildScriptError::Message(rstd::format(
                "runtime resource path '{}' is not valid UTF-8", relative.as_path())));
        }
...
auto runtime_resource_directory_identity(ref<rstd::path::Path> root)
    -> BuildScriptResult<String> {
    ...
    rstd::slice_::sort_unstable_by(files.as_mut_slice().as_mut_ref(),
                                   [](const PathBuf& left, const PathBuf& right) {
                                       return left.as_path().to_string_lossy() <
                                              right.as_path().to_string_lossy();
                                   });
    if (files.is_empty()) {
...
auto resolve_runtime_resources(const PackageMetadata&          metadata,
    ...
    for (const auto& target : metadata.targets) {
        ...
        for (const auto& declaration : target.runtime_resources) {
            ...
            if (inspected->is_symlink() || ! inspected->is_dir()) {
                return Err(BuildScriptError::Message(rstd::format(
                    "runtime resource '{}' for target '{}::{}' must be a directory",
                    requested.as_path(),
            ...
            if (canonical->as_path().strip_prefix(package_root.as_path()).is_none()) {
                return Err(BuildScriptError::Message(
                    rstd::format("runtime resource '{}' escapes generated package root",
                                 requested.as_path())));
            }
            auto files = Vec<PathBuf>::make();

...
public:
    ...
    static auto create(usize jobs, usize max_in_flight) -> BuildResult<FrontendScanExecutor> {
        if (jobs == usize {} || max_in_flight == usize {}) {
            return Err(BuildError::Message(String::make("scan execution requires non-zero jobs and capacity"_str)));
        }
        ...
        if (pool.is_err()) {
            return Err(BuildError::System(SystemError::Io(
                String::make("create scan worker pool"_str),
                PathBuf::make(),
        ...
        if (tasks.is_err()) {
            return Err(BuildError::System(SystemError::Io(
                String::make("create scan task set"_str),
                PathBuf::make(),
    ...
    auto recv() -> BuildResult<FrontendScanTaskResult> {
        ...
        if (result.is_none()) {
            return Err(BuildError::Message(
                String::make("scan task completed without a result"_str)));
        }
...
public:
    ...
    auto submit(usize node, FrontendAnalysisTask task) -> BuildResult<empty> {
        if (executor_.is_none()) {
            return Err(BuildError::Message(
                String::make("scan execution has no prepared frontier"_str)));
        }

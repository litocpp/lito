...
export module lito.build.host_tool_error_contract;
...
class HostBuildToolError {
    RSTD_ENUM(HostBuildToolError,
              ...
              (Version,
               (String alias; String expected; String actual; PathBuf executable;)))
};

template<typename T>
using HostBuildToolResult = rstd::Result<T, HostBuildToolError>;
...
template<>
struct Impl<convert::From<lito::SystemError>, lito::HostBuildToolError> {
    static auto from(lito::SystemError error) -> lito::HostBuildToolError {
...
struct Impl<fmt::Display, lito::HostBuildToolError> : ImplBase<lito::HostBuildToolError> {
    auto fmt(fmt::Formatter& formatter) const -> bool {
        ...
        if (error.is_Source()) return formatter.write_str("host build-tool source acquisition failed"_str);
        ...
        if (error.is_UnsupportedHost()) {
            ...
            return formatter.write_str(rstd::format(
                "build-tool '{}' has no archive for host {}-{}",
                value.alias.as_str(), value.os.as_str(), value.architecture.as_str()).as_str());
        }
        if (error.is_MissingExecutable()) {
            const auto& value = error.as_MissingExecutable();
            return formatter.write_str(rstd::format(
                "build-tool '{}' executable '{}' is missing or invalid",
                value.alias.as_str(), value.path.as_path()).as_str());
        }
        const auto& value = error.as_Version();
        return formatter.write_str(rstd::format(
            "build-tool '{}' expected version '{}', received '{}' from '{}'",
            value.alias.as_str(), value.expected.as_str(), value.actual.as_str(),
            value.executable.as_path()).as_str());
    }
};

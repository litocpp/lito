...
export module lito.build.host_tool;
...
auto host_tool_io_failure(ref<str>                operation,
    ...
    return Err(HostBuildToolError::System(SystemError::Io(
...
auto executable_digest(ref<rstd::path::Path> path) -> HostBuildToolResult<String> {
    ...
    if (data.is_err()) {
        return Err(HostBuildToolError::System(SystemError::Io(
            String::make("read host build-tool executable"_str),
            PathBuf::from(path),
...
auto host_tool_receipt_identity(const PackageBuildToolRequirement& owned,
...
auto host_tool_receipt_matches(ref<rstd::path::Path>             receipt,
                               const PackageBuildToolRequirement& owned,
                               const BuildToolArchiveManifest&    archive,
                               const HostInfo&                    host,
                               ref<str>                           source_identity,
                               ref<str>                           executable_digest,
                               ref<str>                           receipt_identity)
    -> HostBuildToolResult<bool> {
    ...
    if (contents.is_err()) {
        ...
        return host_tool_io_failure<bool>("read host build-tool receipt"_str,
                                          receipt,
                                          rstd::move(error));
    }
...
auto write_host_tool_receipt(ref<rstd::path::Path>              receipt,
    ...
    document.insert(String::make("alias"_str),
                    Json::String(owned.requirement.alias.clone()));
    ...
    if (written.is_err()) {
        return host_tool_io_failure<empty>("write host build-tool receipt"_str,
                                           receipt,
...
void emit_host_tool(BuildObserver             observer,
                    BuildEventKind            kind,
                    ref<str>                  alias,
...
auto select_host_build_tool_archive(const BuildToolRequirement& requirement,
                                    const HostInfo&             host)
    -> HostBuildToolResult<ref<BuildToolArchiveManifest>> {
    for (const auto& archive : requirement.archives) {
        if (archive.host.os == host.os.as_str() &&
            archive.host.architecture == host.architecture) {
            return Ok(ref<BuildToolArchiveManifest>::from_raw_parts(rstd::addressof(archive)));
        }
    }
    return Err(HostBuildToolError::UnsupportedHost(requirement.alias.clone(),
                                                    host.os.clone(),
                                                    host.architecture.name.clone()));
}
...
auto resolve_host_build_tools(const PackageMetadata&             metadata,
    ...
    auto requirements = Vec<ref<PackageBuildToolRequirement>>::make();
    ...
    for (const auto& owned : metadata.build_tools) {
        ...
        for (const auto& existing : requirements) {
            if (existing->requirement.alias == owned.requirement.alias.as_str()) {
                return Err(HostBuildToolError::DuplicateAlias(
                    owned.requirement.alias.clone()));
            }
        }
        auto archive = rstd_try(select_host_build_tool_archive(owned.requirement, host));
        requirements.push(
            ref<PackageBuildToolRequirement>::from_raw_parts(rstd::addressof(owned)));
    ...
    auto cmake_tool = rstd_try(resolver.resolve(cmake.executable.as_path(),
                                                "CMake archive extractor"_str));
    ...
    for (usize index {}; index < requirements.len(); ++index) {
        ...
        auto key = rstd::crypto::sha256_hex(
        ...
        if (created.is_err()) {
            return host_tool_io_failure<ResolvedHostBuildTools>(
                "create host build-tool store"_str,
                area.as_path(),
        ...
        if (opened.is_err()) {
            return host_tool_io_failure<ResolvedHostBuildTools>(
                "open host build-tool lock"_str,
                lock_path.as_path(),
        ...
        if (locked.is_err()) {
            return host_tool_io_failure<ResolvedHostBuildTools>(
                "lock host build-tool store"_str,
                lock_path.as_path(),
                rstd::move(locked).unwrap_err());
        }
        auto digest = rstd_try(executable_digest(canonical->as_path()));
        auto receipt_identity = host_tool_receipt_identity(owned,
                                                          archive,
                                                          host,
                                                          acquired[index].identity.as_str(),
                                                          digest.as_str());

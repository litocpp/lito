...
export module lito.install.selection;
...
auto resolve_install_packages(const ResolvedPackageSelection& selection,
    ...
    for (const auto& name : selection.install_package_names) {
        ...
        if (! package->manifest.target.matches(target)) {
            return selection_failure<Vec<PackageInstallInput>>(rstd::format(
                "install package '{}' does not support target '{}'", name.as_str(), target.triple.as_str()));
        }
        ...
        for (usize artifact_index {}; artifact_index < recipe.artifacts.len(); ++artifact_index) {
            ...
            if (artifact.target.package != recipe.owner.as_str()) {
                return selection_failure<InstallBuildRequirements>(rstd::format(
                    ...
                    recipe.owner.as_str(), artifact.target.package.as_str()));
            }

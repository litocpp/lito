...
export module lito.install.script_error_contract;
...
template<typename T>
using InstallScriptResult = rstd::Result<T, InstallScriptError>;
    ...
    auto fmt(fmt::Formatter& formatter) const -> bool {
        ...
        if (error.is_Io()) {
            return formatter.write_str(rstd::format("cannot {} '{}'",
                                                    ...
                                                    error.as_Io().path.as_path()).as_str());
        }
        if (error.is_Lua()) {
        ...
        if (error.is_Binding()) {
            return formatter.write_str(
                rstd::format("install script '{}' has an invalid recipe",
                             error.as_Binding().path.as_path()).as_str());
        }
        if (error.is_Template()) {
            return formatter.write_str(
                rstd::format("cannot render install template '{}'",
                             error.as_Template().path.as_path()).as_str());
        }

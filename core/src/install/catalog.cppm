...
export module lito.install.catalog;
...
auto package_info_json(const InstallPackageInfo& info) -> InstallStoreResult<Json> {
    ...
    package.insert(String::make("id"_str), json_string(info.identity.id.as_str()));
    package.insert(String::make("name"_str), json_string(info.identity.name.as_str()));
    package.insert(String::make("version"_str), json_string(info.version.as_str()));
    package.insert(String::make("source"_str),
                   rstd_try(serialize_install_source_provenance(info.provenance)));
    package.insert(String::make("profile"_str), json_string(info.profile.as_str()));
    package.insert(String::make("target"_str), json_string(info.target.as_str()));
    ...
    for (const auto& entry : info.entries) {
        ...
        item.insert(
            ...
            json_string(entry.kind == InstallOwnedEntryKind::File ? "file"_str : "soft-link"_str));
    ...
    for (const auto& dependency : info.runtime_dependencies) {
        auto item = JsonMap::make();
        item.insert(String::make("package-id"_str), json_string(dependency.package_id.as_str()));
        item.insert(String::make("name"_str), json_string(dependency.name.as_str()));
        item.insert(String::make("source"_str), json_string(dependency.source_identity.as_str()));
    ...
    root.insert(String::make("layout"_str),
                json_string(info.layout == InstallManagedPackageLayout::DirectBin
                                ? "direct-bin"_str

...
auto source_file(const InstallRecipe& recipe, ref<rstd::path::Path> relative)
    ...
    if (metadata.is_err()) {
        return materialize_failure<PathBuf>(
            rstd::format("cannot inspect package file '{}': {}",
                         requested.as_path(), rstd::move(metadata).unwrap_err()));
    }
    ...
    if (canonical.is_err()) {
        return materialize_failure<PathBuf>(
            rstd::format("cannot resolve package file '{}': {}",
                         requested.as_path(), rstd::move(canonical).unwrap_err()));
    }
...
auto append_entry(Vec<InstallEntry>& entries, InstallEntry entry)
    ...
    for (const auto& prior : entries) {
        if (prior.relative_destination.as_path() == entry.relative_destination.as_path()) {
            return materialize_failure<empty>(rstd::format(
                "install destination '{}' is declared more than once",
...
auto artifact_for(const BuildSummary& summary, const PackageTargetId& target)
    ...
    if (result == nullptr || result->kind != ArtifactKind::Executable) {
        return materialize_failure<const BuiltArtifact*>(rstd::format(
            "build did not return an executable artifact for '{}'", package_target_id_text(target)));
    }
    return Ok(result);
}
...
auto asset_set_for(const ExternalAssetCatalog& catalog, ref<str> dependency, ref<str> name)
    ...
    for (const auto& set : catalog.sets) {
        ...
        if (result != nullptr) {
            return materialize_failure<const ExternalAssetSet*>(rstd::format(
...
auto relative_text(ref<rstd::path::Path> destination, ref<rstd::path::Path> base) -> String {
    auto destination_text = destination.to_str();
    auto base_text = base.to_str();
    if (destination_text.is_none() || base_text.is_none()) return destination.to_string_lossy();
    auto base_parts = Vec<ref<str>>::make();
    auto remaining = *base_text;
    while (! remaining.is_empty()) {
        auto part = remaining.split_once("/"_str);
        auto value = part.is_some() ? part->get<0>() : remaining;
        base_parts.push(rstd::move(value));
        remaining = part.is_some() ? part->get<1>() : ""_str;
    }
    auto destination_parts = Vec<ref<str>>::make();
    remaining = *destination_text;
    while (! remaining.is_empty()) {
        auto part = remaining.split_once("/"_str);
        auto value = part.is_some() ? part->get<0>() : remaining;
        destination_parts.push(rstd::move(value));
        remaining = part.is_some() ? part->get<1>() : ""_str;
    }
    auto common = usize();
    while (common < base_parts.len() && common < destination_parts.len() &&
           base_parts[common] == destination_parts[common]) {
        ++common;
    }
    ...
    for (auto index = common; index < destination_parts.len(); ++index) {
        if (! result.is_empty()) result.push_ascii('/');
        result.push_str(destination_parts[index]);
    }
    return result;
}
...
auto sort_entries(Vec<InstallEntry>& entries) -> void {
    rstd::slice_::sort_unstable_by(entries.as_mut_slice().as_mut_ref(),
                                   [](const InstallEntry& left, const InstallEntry& right) {
                                       return install_path_less(left.relative_destination.as_path(),
...
auto materialize_install_plan(Vec<InstallRecipe> recipes,
    ...
    for (auto& recipe : recipes) {
        ...
        for (const auto& file : recipe.files) {
            ...
            rstd_try(append_entry(entries, InstallEntry {
                .origin = InstallEntryOrigin::PackageFile(recipe.owner.clone(), file.source.clone()),
        ...
        for (const auto& requested : recipe.external_assets) {
            ...
            for (const auto& asset : set->entries) {
                ...
                rstd_try(append_entry(entries, InstallEntry {
                    .origin = InstallEntryOrigin::ExternalAsset(
                        requested.dependency.clone(), requested.set.clone(), asset.logical_path.clone()),
        ...
        for (const auto& configured : recipe.templates) {
            ...
            if (contents.is_err()) {
                return materialize_failure<InstallPlan>(rstd::format(
                    "cannot read install template '{}': {}",
                    input.as_path(), rstd::move(contents).unwrap_err()));
            }
            auto rendered = render_configure_template(
                contents->as_str(), configured.values, input.as_path());
            if (rendered.is_err()) {
                return materialize_failure<InstallPlan>(rstd::format(
                    "cannot render install template '{}': {}",
                    input.as_path(), rstd::move(rendered).unwrap_err()));
            }
        ...
        for (const auto& inventory : recipe.inventories) {
            ...
            for (const auto& entry : entries) {
                text.push_str(relative_text(entry.relative_destination.as_path(),
                                            inventory.relative_to.as_path()).as_str());
                text.push_ascii('\n');
            }

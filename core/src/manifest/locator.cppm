...
auto try_locate_manifest(ref<rstd::path::Path> requested_directory)
    ...
    if (canonical_directory.is_err()) {
        return locator_failure<Option<ManifestLocation>>(rstd::format(
            "cannot resolve manifest directory '{}': {}",
            requested_directory,
    ...
    if (metadata.is_err()) {
        return locator_failure<Option<ManifestLocation>>(rstd::format(
            "cannot inspect manifest directory '{}': {}",
            directory.as_path(),
            rstd::move(metadata).unwrap_err()));
    }
    if (! metadata->is_dir()) {
        return locator_failure<Option<ManifestLocation>>(rstd::format(
            "manifest root '{}' is not a directory", directory.as_path()));
    }

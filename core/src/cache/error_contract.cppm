...
export module lito.cache.error_contract;
...
class CacheError {
    RSTD_ENUM(CacheError,
              (Record, (rstd::string::String message;)),
              (Io,
...
template<typename T>
using CacheResult = rstd::Result<T, CacheError>;
...
struct Impl<fmt::Display, lito::CacheError> : ImplBase<lito::CacheError> {
    auto fmt(fmt::Formatter& formatter) const -> bool {
        ...
        return formatter.write_fmt(
            fmt::Arguments::make("cannot {} cache path '{}'", value.operation, value.path.as_path()));
    }
};

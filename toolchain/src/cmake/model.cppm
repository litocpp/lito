...
auto work_area(const ResolvedCMakeDependencyRequirement&    requirement,
    ...
    append_identity(recipe, cpp::cpp_optimization_option(profile.cpp.codegen.optimization));
    append_identity(recipe, cpp::cpp_debug_option(profile.cpp.codegen.debug_info));
    append_identity(recipe, cpp::cpp_lto_option(profile.cpp.codegen.lto));
    for (const auto& entry : requirement.cache) {
